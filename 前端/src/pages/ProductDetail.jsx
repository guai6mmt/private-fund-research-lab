...
function PageProductDetail({ productId, onNav, openDrawer, setProductId }) {
  ...
  return (
    <React.Fragment>
      ...
      <div className="grid" style={{ gridTemplateColumns: "minmax(0, 1fr) minmax(0, 1fr)", gap: 14, marginBottom: 16 }}>
        {/* === 业绩表现 === */}
        <div className="card">
          ...
          <div className="card-body" style={{ padding: 0 }}>
            <div style={{ padding: "14px 18px 6px" }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: "var(--fg-2)", display: "flex", alignItems: "center", gap: 6, marginBottom: 10 }}>
                ...
                净值走势
                <span className="muted" style={{ fontWeight: 400, fontSize: 11 }}>产品 vs 基准 vs 同类中位数</span>
              </div>
            ...
            <div style={{ padding: "12px 18px 16px" }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: "var(--fg-2)", display: "flex", alignItems: "center", gap: 6, marginBottom: 10 }}>
                <span style={{ width: 4, height: 4, borderRadius: 2, background: "var(--primary)" }} />
                收益指标
                <span className="muted" style={{ fontWeight: 400, fontSize: 11 }}>累计与年化口径</span>
              </div>
              {(() => {
                const navArr = p.navSeries;
                const last = navArr[navArr.length - 1];
                const startVal = navArr[0];
                const weekRef = navArr[Math.max(0, navArr.length - 6)];
                const weekChg = last / weekRef - 1;
                const sinceChg = last / startVal - 1;
                const items = [
                  { label: "累计净值", val: last.toFixed(4), color: "var(--fg)" },
                  ...
                  { label: "成立以来年化收益", val: (p.annualReturn * 100).toFixed(2) + "%", color: p.annualReturn >= 0 ? "var(--red)" : "var(--green)" },
                ];
                return (
            ...
            <div style={{ padding: "12px 18px 16px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 10 }}>
                {[
                  ...
                  { label: "夏普比率", val: p.sharpe.toFixed(2), color: "var(--primary)" },
                  { label: "卡玛比率", val: p.calmar.toFixed(2), color: "var(--purple)" },
                ].map((m, i) => (
                  <div key={i} className="s-metric">
                    <div className="l">{m.label}</div>
                    <div className="v" style={{ color: m.color }}>{m.val}</div>
